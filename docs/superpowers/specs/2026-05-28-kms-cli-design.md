...
- `kms me`：查询当前用户信息。
- `kms spaces`：获取当前用户有权限访问的知识库和渠道信息。知识库和渠道是父子关系，一个知识库下可以有多个渠道。
- `kms faqs <channel_id>`：获取指定渠道下的 FAQ 列表。
...
- Client 层：统一处理 HTTP 请求、接口路径拼接和路径参数替换。
